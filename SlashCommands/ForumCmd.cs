    ...
    public class ForumCmd : InteractionModuleBase
    {
        ...
        public async Task UnpinMessage(IMessage message)
        {
            ...
            if (threadChannel != null)
            {
                ...
                var firstPinnedMessage = pinnedMessages.OrderBy(x => x.CreatedAt).FirstOrDefault();
                //Is it the owner
                if (firstPinnedMessage != null && firstPinnedMessage.Author.Id == Context.User.Id && message.Id != firstPinnedMessage.Id)
                {
                    //Pin the message
            ...
            await RespondAsync("This command can only be used in threads you own and not on the first message", ephemeral: true);
        }
    }
}
