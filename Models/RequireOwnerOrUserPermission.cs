        ...
        ICommandInfo commandInfo, IServiceProvider services)
    {
        ...
        if (GuildPermission.HasValue)
        {
            if (guildUser == null)
                return await Task.FromResult(
                    PreconditionResult.FromError(NotAGuildErrorMessage ?? "Command must be used in a guild channel."));
            if (!guildUser.GuildPermissions.Has(GuildPermission.Value))
                return await Task.FromResult(PreconditionResult.FromError(ErrorMessage ??
                                                                          $"User requires guild permission {GuildPermission.Value}."));
        }
        ...
        if (ChannelPermission.HasValue)
        {
            ...
            if (!perms.Has(ChannelPermission.Value))
                return await Task.FromResult(PreconditionResult.FromError(ErrorMessage ??
                                                                          $"User requires channel permission {ChannelPermission.Value}."));
        }
